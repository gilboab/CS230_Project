...
from keras.layers import Dense, Dropout, Activation, LSTM, TimeDistributed, Convolution1D, Bidirectional
...
#    model.add(TimeDistributed(Dense(128), input_shape=(window, 200)))
    ...
    model.add(TimeDistributed(Dropout(drop)))
    model.add(Bidirectional(LSTM(256, return_sequences=True, dropout=drop)))
    model.add(Bidirectional(LSTM(256, return_sequences=False, dropout=drop)))
    model.add(Dense(3, activation='softmax'))
    model.summary()
...
#    test_results(prediction,Y_dev)
    ...
    model.save('lstm_model_th_bidirectional.h5')
